...
@WSRestController(path = "/exp/charging/channel", module = "H5渠道-实时计费规则")
public class ExpChargingChannelPlatformController extends AbstractManagerBaseController {

	@Autowired
	private ExpChargingService expChargingService;

	@OpRequestSelectMapping(op = "selectPage", desc = "H5渠道-实时计费规则-分页查询")
	public List<ExpChargingVo> selectPage(PageModel<ExpChargingAo> ao) {
		return expChargingService.selectPage(ao, ChargingServiceUtil.CHANNELPLATFORM, OrderAttributeConst.ORDER_TYPE_EXPCAR_INSTANT);
	}

	@OpRequestSelectMapping(op = "selectCount", desc = "H5渠道-实时计费规则-数量查询")
	public Integer selectCount(PageModel<ExpChargingAo> ao) {
		return expChargingService.selectCount(ao, ChargingServiceUtil.CHANNELPLATFORM, OrderAttributeConst.ORDER_TYPE_EXPCAR_INSTANT);
	}

	@OpRequestSelectMapping(op = "selectDetils", desc = "H5渠道-实时计费规则-详情查询")
	public ExpChargingVo selectDetils(ExpChargingAo ao) {
		return expChargingService.selectDetils(ao, ChargingServiceUtil.CHANNELPLATFORM, OrderAttributeConst.ORDER_TYPE_EXPCAR_INSTANT);
	}

	@OpRequestUpdateMapping(op = "updateStatus", desc = "H5渠道-实时计费规则-启用/禁用")
	public AnwserCode updateStatus(ExpChargingAo ao) {
		return expChargingService.updateStatus(ao, ChargingServiceUtil.CHANNELPLATFORM, OrderAttributeConst.ORDER_TYPE_EXPCAR_INSTANT);
	}

	@OpRequestUpdateMapping(op = "delete", desc = "H5渠道-实时计费规则-删除")
	public AnwserCode delete(ExpChargingAo ao) {
		return expChargingService.delete(ao, ChargingServiceUtil.CHANNELPLATFORM, OrderAttributeConst.ORDER_TYPE_EXPCAR_INSTANT);
	}

	@OpRequestSaveMapping(op = "save", desc = "H5渠道-实时计费规则-添加")
	public AnwserCode save(ExpChargingOpAo ao) {
		return expChargingService.save(ao, ChargingServiceUtil.CHANNELPLATFORM, OrderAttributeConst.ORDER_TYPE_EXPCAR_INSTANT);
	}

	@OpRequestUpdateMapping(op = "update", desc = "H5渠道-实时计费规则-修改")
	public AnwserCode update(ExpChargingOpAo ao) {
		return expChargingService.update(ao, ChargingServiceUtil.CHANNELPLATFORM, OrderAttributeConst.ORDER_TYPE_EXPCAR_INSTANT);
	}

	@OpRequestUpdateMapping(op = "batchAdjust", desc = "H5渠道-实时计费规则-批量调整配置")
	public AnwserCode batchAdjust(ExpChargingAo ao) {
		return expChargingService.batchAdjust(ao, ChargingServiceUtil.CHANNELPLATFORM, OrderAttributeConst.ORDER_TYPE_EXPCAR_INSTANT);
	}

	@OpRequestUpdateMapping(op = "batchUpdateUpStatus", desc = "H5渠道-实时计费规则-批量启用")
	...
	@OpRequestUpdateMapping(op = "batchUpdateDownStatus", desc = "H5渠道-实时计费规则-批量禁用")
	...
	@OpRequestUpdateMapping(op = "batchUpdateDelStatus", desc = "H5渠道-实时计费规则-批量删除")
	...
	@OpRequestSelectMapping(op = "batchSave", desc = "H5渠道-实时计费规则-批量新增")
	public List<ExpChargingVo> batchSave(ChargingBatchSaveAo ao) {
		return expChargingService.batchSave(ao, ChargingServiceUtil.CHANNELPLATFORM, OrderAttributeConst.ORDER_TYPE_EXPCAR_INSTANT);
	}

	@OpRequestSelectMapping(op = "export", desc = "H5渠道-实时计费规则-导出")
	public AnwserCode export(ExpChargingAo ao) {
		return expChargingService.export(ao, ChargingServiceUtil.CHANNELPLATFORM, OrderAttributeConst.ORDER_TYPE_EXPCAR_INSTANT, FileExportModuleConstant.EXP_CHARGING_CHANNEL_PLATFORM);
	}
