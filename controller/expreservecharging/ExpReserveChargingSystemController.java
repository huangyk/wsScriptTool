...
@WSRestController(path = "/expreserve/charging/system", module = "平台计费-预约计费规则")
public class ExpReserveChargingSystemController extends AbstractManagerBaseController {

	@Autowired
	private ExpChargingService expChargingService;

	@OpRequestSelectMapping(op = "selectPage", desc = "平台计费-预约计费规则-分页查询")
	public List<ExpChargingVo> selectPage(PageModel<ExpChargingAo> ao) {
		...
		return expChargingService.selectPage(ao, ChargingServiceUtil.WANSHUNSYSTEM, OrderAttributeConst.ORDER_TYPE_EXPCAR_RESERVE);
	}

	@OpRequestSelectMapping(op = "selectCount", desc = "平台计费-预约计费规则-数量查询")
	public Integer selectCount(PageModel<ExpChargingAo> ao) {
		ao.getBody().setChannelId(ChargingServiceUtil.WANSHUN_CHANNELID);
		return expChargingService.selectCount(ao, ChargingServiceUtil.WANSHUNSYSTEM, OrderAttributeConst.ORDER_TYPE_EXPCAR_RESERVE);
	}

	@OpRequestSelectMapping(op = "selectDetils", desc = "平台计费-预约计费规则-详情查询")
	public ExpChargingVo selectDetils(ExpChargingAo ao) {
		return expChargingService.selectDetils(ao, ChargingServiceUtil.WANSHUNSYSTEM, OrderAttributeConst.ORDER_TYPE_EXPCAR_RESERVE);
	}

	@OpRequestUpdateMapping(op = "updateStatus", desc = "平台计费-预约计费规则-启用/禁用")
	public AnwserCode updateStatus(ExpChargingAo ao) {
		return expChargingService.updateStatus(ao, ChargingServiceUtil.WANSHUNSYSTEM, OrderAttributeConst.ORDER_TYPE_EXPCAR_RESERVE);
	}

	@OpRequestUpdateMapping(op = "delete", desc = "平台计费-预约计费规则-删除")
	public AnwserCode delete(ExpChargingAo ao) {
		return expChargingService.delete(ao, ChargingServiceUtil.WANSHUNSYSTEM, OrderAttributeConst.ORDER_TYPE_EXPCAR_RESERVE);
	}

	@OpRequestSaveMapping(op = "save", desc = "平台计费-预约计费规则-添加")
	public AnwserCode save(ExpChargingOpAo ao) {
		ao.setChannelId(ChargingServiceUtil.WANSHUN_CHANNELID);
		return expChargingService.save(ao, ChargingServiceUtil.WANSHUNSYSTEM, OrderAttributeConst.ORDER_TYPE_EXPCAR_RESERVE);
	}

	@OpRequestUpdateMapping(op = "update", desc = "平台计费-预约计费规则-修改")
	public AnwserCode update(ExpChargingOpAo ao) {
		ao.setChannelId(ChargingServiceUtil.WANSHUN_CHANNELID);
		return expChargingService.update(ao, ChargingServiceUtil.WANSHUNSYSTEM, OrderAttributeConst.ORDER_TYPE_EXPCAR_RESERVE);
	}

	@OpRequestUpdateMapping(op = "batchAdjust", desc = "平台计费-预约计费规则-批量调整配置")
	public AnwserCode batchAdjust(ExpChargingAo ao) {
		ao.setChannelId(ChargingServiceUtil.WANSHUN_CHANNELID);
		return expChargingService.batchAdjust(ao, ChargingServiceUtil.WANSHUNSYSTEM, OrderAttributeConst.ORDER_TYPE_EXPCAR_RESERVE);
	}

	@OpRequestUpdateMapping(op = "batchUpdateUpStatus", desc = "平台计费-预约计费规则-批量启用")
	...
	@OpRequestUpdateMapping(op = "batchUpdateDownStatus", desc = "平台计费-预约计费规则-批量禁用")
	...
	@OpRequestUpdateMapping(op = "batchUpdateDelStatus", desc = "平台计费-预约计费规则-批量删除")
	...
	@OpRequestSelectMapping(op = "export", desc = "平台计费-预约计费规则-导出")
	public AnwserCode export(ExpChargingAo ao) {
		return expChargingService.export(ao, ChargingServiceUtil.WANSHUNSYSTEM, OrderAttributeConst.ORDER_TYPE_EXPCAR_RESERVE, FileExportModuleConstant.EXP_RESERVE_CHARGING_SYSTEM);
	}
